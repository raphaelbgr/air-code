...
import type { Workspace, CliSession, Session, CliProviderId } from '@air-code/shared';
import { formatRelative, getCliProvider, DEFAULT_CLI_PROVIDER } from '@air-code/shared';
...
export function LauncherDialog({ workspace, onClose, onCreated, backend, cliProvider }: Props) {
  const [cliSessions, setCliSessions] = useState<CliSession[]>([]);
  const [loading, setLoading] = useState(true);
  const [creating, setCreating] = useState(false);
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const provider = getCliProvider(cliProvider ?? workspace.settings?.cliProvider ?? DEFAULT_CLI_PROVIDER);

  const buildResumeCmd = useCallback((sessionId: string) => {
    let cmd = `${provider.binary}`;
    if (provider.resumeFlag) cmd += ` ${provider.resumeFlag} ${sessionId}`;
    if (workspace.settings?.skipPermissions?.[provider.id] && provider.skipPermissionsFlag) cmd += ` ${provider.skipPermissionsFlag}`;
    if (workspace.settings?.cliArgs) cmd += ` ${workspace.settings.cliArgs}`;
    return cmd;
  ...
  useEffect(() => {
    api.workspaces.cliSessions(workspace.id)
      .then(setCliSessions)
  ...
  const createSession = useCallback(async (resumeId?: string) => {
    ...
    setCreating(true);
    try {
      const suffix = backend === 'pty' ? ` (${provider.binary}-ps)` : '';
      const session = await api.sessions.create({
        name: workspace.name + suffix,
        workspacePath: workspace.path,
        type: 'cli',
        backend,
        skipPermissions: workspace.settings?.skipPermissions?.[provider.id] ?? false,
        cliArgs: workspace.settings?.cliArgs,
        cliResumeId: resumeId,
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {/* Resume Previous — only for providers that support resume */}
        {provider.supportsResume && <div className="px-4 pb-4">
          {loading ? (
            ...
            <>
              <div className="text-xs text-text-muted uppercase tracking-wider px-1 py-2">
                Resume Previous ({cliSessions.length})
              </div>
              <div
